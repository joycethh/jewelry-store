...
export default function Navbar() {
  ...
  return (
    <nav className="bg-test w-full shadow transition-all duration-200">
        ...
        //   console.log("category", category);
          return (
            <li 
              ...
              {/* submenu */}
              <ul className="absolute top-full left-0 bg-white text-textDark shadow-lg py-2">
                ...
                //   console.log("subcategory", subcategory);
                  return (
                    <li key={subcategory} className="px-4 py-2 hover: underline cursor-pointer">
                      <Link to={`/products/jewelry-by-${category}/${subcategory.toLowerCase()}`}>{subcategory}</Link>
                    </li>
                  );
                })}
              </ul>
